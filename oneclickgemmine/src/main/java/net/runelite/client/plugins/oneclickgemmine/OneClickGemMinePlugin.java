package net.runelite.client.plugins.oneclickgemmine;
...
@Extension
@PluginDescriptor(
        name = "One Click Gem Mine",
        description = "Click anywhere to mine the gem rocks",
        tags = {"gems", "rocks"},
        enabledByDefault = false
)

@Slf4j
public class OneClickGemMinePlugin extends Plugin
{
    @Inject
    ...
    @Provides
    OneClickGemMineConfig provideConfig(ConfigManager configManager)
    {
        return configManager.getConfig(OneClickGemMineConfig.class);
    }

    private static final int GEM_MINE_REGION = 11410;
    ...
    private void onClientTick(ClientTick event)
    {
        ...
        if (client.getLocalPlayer().getWorldLocation().getRegionID() == GEM_MINE_REGION)
        {
    ...
    {
        if (!event.getMenuOption().equals("One Click Gem Rock"))
        {
            return;
        }
        ...
        GameObject gemrock = new GameObjectQuery()
                .idEquals(ObjectID.ROCKS_11380)
                .result(client)
